...
const integrations = [
  ...
  { name: 'HotelBeds', description: 'Global distribution' },
  { name: 'Webhotelier', description: 'Hotel management' },
  { name: 'Hotelston', description: 'Wholesale platform' },
  { name: 'GIATA', description: 'Hotel content database' },
  { name: 'Expedia Group', description: 'OTA connectivity' },
  { name: 'Emerging Group', description: 'Distribution network' },
];
            ...
            <div className="grid lg:grid-cols-3 gap-8 items-center">
              ...
              >
                ...
                <div className="mt-4 flex flex-wrap justify-center gap-2">
                  {['OTA', 'DMC', 'Agency'].map((type) => (
                    <span 
                      key={type}
                      className="px-2 py-1 text-xs font-mono bg-white/5 rounded border border-white/10"
                    >
                      {type}
                    </span>
                  ))}
              ...
              >
                ...
                <motion.div 
                  className="hidden lg:block absolute top-1/2 -left-6 w-3 h-3 rounded-full bg-accent"
                  animate={{ x: [0, 40, 0], opacity: [0, 1, 0] }}
                  transition={{ duration: 2, repeat: Infinity, ease: 'linear' }}
                />
                <motion.div 
                  className="hidden lg:block absolute top-1/2 -right-6 w-3 h-3 rounded-full bg-accent"
                  animate={{ x: [0, -40, 0], opacity: [0, 1, 0] }}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {integrations.map((integration, index) => (
              <motion.div
                ...
                className="spotlight-card spotlight-border glass rounded-xl p-4 text-center group cursor-pointer relative"
              ...
              >
                ...
                {/* Tooltip */}
                {hoveredPartner === integration.name && (
                  <motion.div
